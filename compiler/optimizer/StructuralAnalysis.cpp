 ...
 */
TR_RegionStructure *TR_RegionAnalysis::findNaturalLoop(StructInfo &node, WorkBitVector &regionNodes,
    WorkBitVector &nodesInPath)
{
    ...
    while (cursor.hasMoreElements()) {
        ...
        if (_dominators.dominates(node._originalBlock, backEdgeNode._originalBlock)) {
            ...
            if (_useNew) {
                addNaturalLoopNodesIterativeVersion(backEdgeNode, regionNodes, nodesInPath, cyclesFound,
                    node._originalBlock);
            } else {
 ...
 * to justify it.
 */
TR_RegionStructure *TR_RegionAnalysis::findRegion(StructInfo &node, WorkBitVector &regionNodes,
    WorkBitVector &nodesInPath)
{
    bool cyclesFound = false;
    regionNodes.empty();
...
void TR_RegionAnalysis::addRegionNodesIterativeVersion(StructInfo &node, WorkBitVector &regionNodes,
    WorkBitVector &nodesInPath, bool &cyclesFound, TR::Block *hdrBlock)
{
    // We will be pushing elements backwards to emulate processing in a depth-first order
    // Also, we will be pushing the same node twice in order to update nodesInPath correctly
...
void TR_RegionAnalysis::addRegionNodes(StructInfo &node, WorkBitVector &regionNodes, WorkBitVector &nodesInPath,
    bool &cyclesFound, TR::Block *hdrBlock)
{
    int32_t index = node._nodeIndex;
